...
class Execute:
    ...
    def __init__(self, lang = "c", name = '', main = '', func = '', input = '', depends = '', testcases = None):
        ...
        if(this_config.get('classname_is_same_as_file_name') is True):
            name = 'Solution'
        ...
        try:
            ...
            self.prog_obj_name = BASE_PATH + name + '.exe' if this_config.get('prog_obj_name') is None else string.Template(this_config['prog_obj_name']).substitute({'name':name})
